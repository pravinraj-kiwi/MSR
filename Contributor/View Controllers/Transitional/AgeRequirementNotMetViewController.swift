//
//  AgeRequirementNotMetViewController.swift
//  Contributor
//
//  Created by John Martin on 1/28/19.
...
class AgeRequirementNotMetViewController: UIViewController {
  struct Layout {
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    setTitle(Text.ageNotMet.localized())
    setupViews()
  }
  ...
  override func setupViews() {
    ...
    actionButton.addTarget(self, action: #selector(self.exitToLogin), for: UIControl.Event.touchUpInside)
  }
  
  @objc func exitToLogin() {
    finish()
  }
  
  func finish() {
    let loginVC = LoginViewController()
    let nav = LoginNavigationViewController(rootViewController: loginVC)
    self.rootViewController?.viewController = nav
  }
}
