...
extension NetworkManager {
    ...
    func checkIfAppConnected(completion: @escaping ([ConnectedAppData]?, Error?) -> Void) -> Cancellable? {
        return provider.request(ContributorAPI.getConnectedAppStatus) {
            [weak self] (result) in
    ...
    func updateLinkedinStatus(adType: String, completion: @escaping (Error?) -> Void) -> Cancellable? {
        return provider.request(ContributorAPI.linkedinAdClicked(adType)) {
            [weak self] (result) in
            guard let _ = self else {
                return
            }
    ...
    func updateAuthenticatedLinkedinUser(linkedinId: String,
        ...
        return provider.request(ContributorAPI.linkedinConnect(parameter)) {
            (result) in
            switch result {
            case .failure(let error):
              completion(error)
    ...
    func updateAuthenticatedFacebookUser(facebookId: String,
        ...
        return provider.request(ContributorAPI.facebookConnect(parameter)) {
            (result) in
            switch result {
            case .failure(let error):
               completion(error)
    ...
    func getLinkedinUserAccessToken(authorizationCode: String,
        ...
        let redirectURL = LinkedinAccounts.redirectURI.rawValue.addingPercentEncoding(withAllowedCharacters: NSCharacterSet.alphanumerics)
    ...
    func getLinkedinUserData(authorizationCode: String,
        ...
        getLinkedinUserAccessToken(authorizationCode: authorizationCode) { (accessToken, error) in
            ...
            Alamofire.request(targetUrl, encoding: JSONEncoding.default, headers: ["Authorization": "Bearer \(accessToken)"])
                .validate()
                .responseJSON {
                    response in
                    switch response.result {
